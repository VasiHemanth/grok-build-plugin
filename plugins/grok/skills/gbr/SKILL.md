...
name: gbr
description: >
  Pair a phone running Build Remote Agent to the local Grok Build desktop session.
...
compatibility: Requires gbr-agent ≥ 0.6.0 on the host. Loopback only. No mailbox keys in this file. grok_search MCP unchanged.
metadata:
  version: "0.6.1"
...
## This host (grok-build-plugin)

This plugin wraps the local `grok` CLI (`grok_search`, `/grok:review`,
`/grok:rescue`). That MCP/slash surface **stays**. Build Remote Agent is an
optional spectator pairing device for the same desktop Grok Build session:
store app + GitHub/HTTPS relay. Do not invent a fourth pair protocol. Do not
put mailbox keys in `.mcp.json`.
